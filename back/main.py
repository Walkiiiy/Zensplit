...
def remove_all_spaces(text):
    # 使用正则表达式匹配所有的空白字符（包括空格、制表符、换行符等）并替换为''
...
def main():
    ...
    i = 0
    while i < len(ocrRes):
        ocrRes[i] = remove_all_spaces(ocrRes[i])
        i += 1
    ...
    for line in lines:
        ...
        try:
            ...
            warnings.warn('警告：可能没有切割完整', UserWarning)
            break
        ...
        for x_ordinate in h_list_Zen:
            ...
            if probability < 0.1:
                ...
                flag = 0  # 保证只进行一次纠错
                # 确认不是第一个字，再比较左侧
                if i != 0 and not flag and ocrRes[row_num][i] not in ignoreChar and ocrRes[row_num][i-1] not in ignoreChar:
                    lProbability = predict.compare(img_path, ((ordinate[0][0], ordinate[1][0]), (
                        ordinate[0][1], ordinate[1][1])), ocrRes[row_num][i-1])
                    if lProbability > 0.5:
                        color = (0, 255, 0)
                        # 字符串相对于字符框向右滑动
                        newOcrRow = ocrRes[row_num][:i]+ocrRes[row_num][i-1:]
                        ocrRes[row_num] = newOcrRow
                        print('right move')
                        flag = 1
                        print(ocrRes[row_num])

                # 确认不是最后一个字，再比较右侧
                if i < len(h_list_Zen)-1 and not flag and ocrRes[row_num][i+1] not in ignoreChar and ocrRes[row_num][i] not in ignoreChar:
                    rProbability = predict.compare(img_path, ((ordinate[0][0], ordinate[1][0]), (
                        ordinate[0][1], ordinate[1][1])), ocrRes[row_num][i+1])
                    if rProbability > 0.5:
                        color = (0, 255, 0)
                        # 字符串相对于字符框向左滑动
                        ...
                        print(ocrRes[row_num])
                # 右侧第二个字
                if i < len(h_list_Zen)-2 and not flag and ocrRes[row_num][i+2] not in ignoreChar and ocrRes[row_num][i] not in ignoreChar:
                    r2Probability = predict.compare(img_path, ((ordinate[0][0], ordinate[1][0]), (
                        ordinate[0][1], ordinate[1][1])), ocrRes[row_num][i+2])
                    if r2Probability > 0.5:
                        color = (0, 255, 0)
                        # 字符串相对于字符框向左滑动
                        newOcrRow = ocrRes[row_num][:i]+ocrRes[row_num][i+2:]
                        ocrRes[row_num] = newOcrRow
                        print('left move 2')
                        print(ocrRes[row_num])
            else:
            ...
            cv2.rectangle(img_bgr, (ordinate[0][0], ordinate[1][0]),
                          (ordinate[0][1], ordinate[1][1]), color, 1)
            # 打印文字
            try:
