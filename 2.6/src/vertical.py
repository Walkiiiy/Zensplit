...
def split_default(list_data, row_num, total_row, char_width):  # 适用于中文和大部分标点
    ...
    # 分裂检查
    try:
        is_more = check_if_more(
            list_data, row_num, char_width, v_list[-1]-v_list[0])  # 传参oldlength时出错
    except:
        is_more = 0
    if is_more:
        while is_more:
            ...
            v_list += to_be_add
            is_more = check_if_more(
                list_data, row_num, char_width, v_list[-1]-v_list[0])
...
def get_vvList_Zen(list_data, char_width, ocr_content):
    ...
    char_width_eng_low = (
        char_width[1]*0.2, char_width[1]*0.475, char_width[1]*0.7)
    char_width_eng_up = (
        char_width[1]*0.4, char_width[1]*0.55, char_width[1]*0.8)
    ...
    while row_num < total_row:
        ...
        if ocr_content[char_num] in string.ascii_lowercase:
            v_list, row_num = split_by_range(
                list_data, row_num, char_width_eng_low)
        elif ocr_content[char_num] in string.ascii_uppercase:
            v_list, row_num = split_by_range(
                list_data, row_num, char_width_eng_up)
        elif ocr_content[char_num] in string.digits:  # 数字进行纯投影
            v_list, row_num = split_pure_projection(list_data, row_num)
        elif ocr_content[char_num] == '-' or ocr_content[char_num] == '-' or ocr_content[char_num] == '_' or ocr_content[char_num] == '”':
            v_list, row_num = split_by_range(
                list_data, row_num, char_width_dash)
